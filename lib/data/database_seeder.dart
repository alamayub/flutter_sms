...
class DatabaseSeeder {
  ...
  static Future<void> seedIfEmpty(AppDatabase db) async {
    ...
    final existingEmployees = await db.getAllEmployees();
    if (existingEmployees.isEmpty) {
      await seedEmployees(db);
    }

    final existingPeriods = await db.getAllPeriodsWithDetails();
    if (existingPeriods.isEmpty) {
      await seedTimetable(db);
    }

    final existingContacts = await db.getAllContacts();
    ...
    if (existingStudents.isEmpty || existingHistories.isEmpty) {
      await seedStudents(db);
    }

    final existingCategories = await db.getAllExpenseCategories();
    if (existingCategories.isEmpty) {
      await seedExpenseCategories(db);
    }

    final existingExpenses = await db.getExpensesWithCategory();
    if (existingExpenses.isEmpty) {
      await seedExpenses(db);
    }

    final existingPayroll = await db.getSalaryPaymentsWithDetails();
    if (existingPayroll.isEmpty) {
      await seedPayroll(db);
    }

    final existingFees = await db.getAllFeeCategories();
    ...
    final existingExamResults = await db.select(db.examResults).get();
    if (existingExamResults.isEmpty) {
      await seedExamResults(db);
    }
  }
